...
CELLS = [
  {
    "cell_type": "markdown",
    "metadata": {},
    "source": "# TC1 V4 · SECOP Data Pipeline\n\n## De una API pública a un producto de datos multimodelo\n\n**Curso:** Big Data · Maestría en Analítica de Datos  \n**Modalidad:** parejas · **100 puntos**\n\nEste taller evalúa una ruta completa: **adquisición → concurrencia → calidad → trazabilidad → MongoDB → Cassandra → Neo4j → comunicación técnica**.\n\n### Pregunta profesional\n\n> ¿Puede el equipo construir una adquisición reproducible de SECOP, demostrar que su versión concurrente recupera los mismos datos que la secuencial y convertir ese snapshot en modelos útiles para preguntas distintas?\n\n### Regla de evidencia\n\nAbrir Colab, ejecutar una celda o mostrar una captura **no equivale a demostrar aprendizaje**. La evidencia es el conjunto de artefactos + variables + resultados que verifica el validador.\n\n```text\nSECOP II API\n  ├─ Procesos p6dx-8zbt\n  └─ Contratos jbjy-vk9h\n        ↓\nsecuencial ↔ ThreadPoolExecutor\n        ↓\nRAW + manifest + hash + calidad\n        ↓\nmodelo documental + Atlas idempotente\n        ↓\nbandeja → Cassandra → Neo4j\n        ↓\ndecision_log + informe + manifest_tc1.json\n```"
  },
  {
    "cell_type": "markdown",
    "metadata": {},
    "source": "## Rúbrica\n\n| Etapa | Evidencia | Puntos |\n|---|---|---:|\n| E1 | API real, SoQL, secuencial/concurrente, trazabilidad y calidad | 20 |\n| E2 | modelo documental, Atlas real, upsert e índices | 30 |\n| E3 | producto analítico desde Atlas | 10 |\n| E4 | Cassandra query-first | 15 |\n| E5 | Neo4j y contexto relacional | 20 |\n| E6 | decisiones, informe, reproducibilidad y paquete | 5 |\n| **Total** | | **100** |\n\n**No se califica que 4 hilos sean “X veces más rápidos”.** Se califica que la comparación sea válida, que ambos caminos produzcan el mismo snapshot y que puedan justificar la configuración."
  },
  {
    "cell_type": "code",
  ...
  },
  {
    ...
    "metadata": {},
    "outputs": [],
    "source": "MAX_WORKERS = 4\n\ndef descargar_concurrente(endpoint_id, *, select, where, order, max_rows, page_size=PAGE_SIZE, max_workers=MAX_WORKERS):\n    # TODO:\n    # 1. construir offsets\n    # 2. enviar fetch_page con ThreadPoolExecutor\n    # 3. reunir resultados por offset\n    # 4. concatenar respetando offset y recortar max_rows\n    # Debe retornar (DataFrame, lista_metadata)\n    return None, []\n\n# TODO: ejecutar procesos con MAX_WORKERS\nprocesos_df = None\nmeta_procesos = []\ntiempo_threads = None"
  },
  {
    "cell_type": "code",
    "execution_count": null,
  ...
  },
  {
    ...
    "metadata": {},
    "outputs": [],
    "source": "# CHALLENGE · limpieza e integración\n# 1. conviertan fechas y numéricos con errors=\"coerce\"\n# 2. normalicen adjudicado a booleano\n# 3. agreguen contratos por proceso_de_compra\n# 4. construyan historico: una fila por id_proceso\n# 5. construyan documentos: lista de dicts anidados\n\nhistorico = None\ndocumentos = None\n\n# TODO: guardar snapshot integrado\n# historico.to_parquet(OUT / \"02_secop_integrado.parquet\", index=False)\n# (OUT / \"02_modelo_documental.json\").write_text(...)"
  },
  {
    "cell_type": "markdown",
    "metadata": {},
    "source": "### E2.2 · Atlas real e idempotencia\n\nConéctense con `getpass()`. No guarden credenciales.\n\nEn vez de `delete_many()+insert_many()`, usen:\n\n- índice único por `id_proceso`;\n- `bulk_write`;\n- `UpdateOne(..., upsert=True)`.\n\nEjecuten la carga **dos veces** y demuestren que el número de documentos no aumenta."
  },
  {
    "cell_type": "code",
    "execution_count": null,
    "metadata": {},
    "outputs": [],
    "source": "from pymongo import MongoClient, UpdateOne, ASCENDING, DESCENDING\n\natlas_ping = False\natlas_server_version = None\ncoleccion = None\natlas_idempotencia = {}\natlas_indexes = []\n\n# TODO:\n# 1. pedir plantilla SRV/usuario/password\n# 2. client.admin.command(\"ping\")\n# 3. db = client[\"tc1_bigdata_v4\"]\n# 4. coleccion = db[f\"secop_pipeline_{PAREJA_ID}\"]\n# 5. crear índice único id_proceso\n# 6. bulk_write con upsert=True\n# 7. registrar count primera y segunda ejecución\n# 8. crear al menos un índice compuesto útil para las consultas\n\n# Ejemplo de operación (adapten la lista):\n# operaciones = [UpdateOne({\"id_proceso\": d[\"id_proceso\"]}, {\"$set\": d}, upsert=True) for d in documentos]"
  },
  {
    "cell_type": "markdown",
    "metadata": {},
    "source": "### E2.3 · Consultas documentales\n\nConstruyan:\n\n**Consulta A:** procesos con `precio_base > 0` y al menos un contrato asociado.  \n**Consulta B:** top 10 por valor total de contratos, desempate por `id_proceso ASC`.\n\nLa evidencia debe guardar filtros/proyección/resultado, no credenciales."
  },
  {
    "cell_type": "code",
    "execution_count": null,
...
def build():
    cells=[]
    for c in CELLS:
        src=c["source"].splitlines(keepends=True)
        if c["source"] and (not src or not src[-1].endswith("\n")):
            pass
        cell={"cell_type":c["cell_type"],"metadata":{},"source":src}
        if c["cell_type"]=="code":
