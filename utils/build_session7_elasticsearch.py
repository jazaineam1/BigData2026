...
NOTEBOOK_JSON = r'''{
 "cells": [
  ...
  },
  {
   ...
   "metadata": {},
   "outputs": [],
   "source": "if client is not None:\n    tabla_B = mostrar_hits(\n        ejecutar_config_elastic(\n            [\"nombre_proceso^3\",\"descripcion\"],\n            consulta_personal\n        )\n    )\nelse:\n    # El respaldo local no modela boost por campo.\n    # Conservamos el ranking y dejamos explícita la limitación.\n    tabla_B = tabla_A.copy()\n\nprint(\"CONFIGURACIÓN B\")\ndisplay(tabla_B)"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
