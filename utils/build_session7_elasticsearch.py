# -*- coding: utf-8 -*-
"""Genera la sesión 7 integral: Elasticsearch, BM25, despliegue y reto editorial.
...
NOTEBOOK_JSON = r'''{
 "cells": [
  ...
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "---\n# H · Reto de transferencia — Sala de redacción\n\nHasta ahora trabajaste con procesos contractuales. Ahora debes demostrar que aprendiste **Elasticsearch**, no un caso particular.\n\nVas a reutilizar las **987 noticias reales** que ya conociste en sesiones anteriores, pero con un problema totalmente distinto:\n\n> Eres parte de la mesa digital de un medio. Un editor necesita encontrar rápidamente cinco antecedentes útiles sobre un tema, pero solo puede recomendar artículos de acceso abierto. Tu tarea es construir y evaluar el ranking.\n\nNo hay Laura, contratos ni SECOP en este reto. La misma tecnología debe transferirse a otro dominio.\n\n### Lo que debes demostrar\n\n1. indexar documentos con una estructura diferente;\n2. distinguir campos de búsqueda de campos de filtro;\n3. construir una búsqueda editorial con boosts;\n4. usar <code>premium = false</code> como restricción de negocio;\n5. evaluar si tu tuning mejora <code>Precision@5</code>."
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## H1. Recupera las noticias de sesiones anteriores\n\nEl dataset contiene título, subtítulo, cuerpo, etiquetas, sección, categoría, fecha, acceso premium y URL.\n\nEl texto del cuerpo viene como una lista de bloques. Para Elasticsearch lo convertimos en un campo textual limpio sin perder los metadatos que sí queremos filtrar."
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "metadata": {},
   "outputs": [],
   "source": "URL_NOTICIAS = \"https://raw.githubusercontent.com/jazaineam1/BigData2026/main/Datos/noticias_contratacion_2026.json\"\n\nnoticias_raw = pd.read_json(URL_NOTICIAS)\n\ndef cuerpo_a_texto(bloques):\n    piezas = []\n    if isinstance(bloques, list):\n        for bloque in bloques:\n            if isinstance(bloque, dict) and bloque.get(\"texto\"):\n                piezas.append(str(bloque[\"texto\"]))\n    return \" \".join(piezas)\n\ndef etiquetas_a_texto(etiquetas):\n    if not isinstance(etiquetas, list):\n        return \"\"\n    return \" \".join(\n        str(e.get(\"nombre\",\"\"))\n        for e in etiquetas\n        if isinstance(e, dict)\n    )\n\nnoticias = pd.DataFrame({\n    \"id_noticia\": noticias_raw[\"_id\"].astype(str),\n    \"titulo\": noticias_raw[\"titulo\"].fillna(\"\").astype(str),\n    \"subtitulo\": noticias_raw[\"subtitulo\"].fillna(\"\").astype(str),\n    \"cuerpo\": noticias_raw[\"cuerpo\"].map(cuerpo_a_texto),\n    \"etiquetas_texto\": noticias_raw[\"etiquetas\"].map(etiquetas_a_texto),\n    \"categoria\": noticias_raw[\"categoria\"].fillna(\"\").astype(str),\n    \"seccion\": noticias_raw[\"seccion\"].fillna(\"\").astype(str),\n    \"premium\": noticias_raw[\"premium\"].fillna(False).astype(bool),\n    \"publicado\": pd.to_datetime(noticias_raw[\"publicado\"], errors=\"coerce\").astype(str),\n    \"url\": noticias_raw[\"url\"].fillna(\"\").astype(str),\n}).drop_duplicates(\"id_noticia\").reset_index(drop=True)\n\nnoticias[\"texto_busqueda\"] = (\n    noticias[\"titulo\"] + \" \" +\n    noticias[\"subtitulo\"] + \" \" +\n    noticias[\"etiquetas_texto\"] + \" \" +\n    noticias[\"cuerpo\"]\n).str.strip()\n\nprint(\"Noticias únicas:\", len(noticias))\nprint(\"Categorías:\", noticias[\"categoria\"].nunique())\nprint(\"Acceso abierto:\", int((~noticias[\"premium\"]).sum()))\ndisplay(noticias[[\"id_noticia\",\"titulo\",\"categoria\",\"premium\"]].head(5))"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "### Cómo se lee\n\n**Qué cambió respecto al caso anterior.** Ahora el documento es una noticia, no un proceso. Hay texto largo, etiquetas, fecha y una regla editorial de acceso.\n\n**Qué se mantiene.** Seguimos necesitando mapping, analyzer, bulk, query, filter, score y evaluación.\n\n**Transferencia esperada.** Si el diseño solo funcionaba para contratos, no habías aprendido todavía el patrón general."
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## H2. Crea un índice separado para noticias\n\nNo reutilizamos el índice contractual porque representa otro tipo de documento.\n\nAquí <code>premium</code> es booleano y <code>publicado</code> es fecha. Título, subtítulo, etiquetas y cuerpo son campos de búsqueda."
  },
  {
   "cell_type": "code",
   "execution_count": null,
  ...
  },
  {
   ...
   "metadata": {},
   "outputs": [],
   "source": "docs_news = noticias[\n    [\"id_noticia\",\"titulo\",\"subtitulo\",\"cuerpo\",\"etiquetas_texto\",\n     \"categoria\",\"seccion\",\"premium\",\"publicado\",\"url\"]\n].to_dict(\"records\")\n\nif client is not None:\n    acciones_news = (\n        {\n            \"_index\": INDEX_NEWS,\n            \"_id\": d[\"id_noticia\"],\n            \"_source\": d\n        }\n        for d in docs_news\n    )\n    ok_news, errores_news = bulk(\n        client,\n        acciones_news,\n        refresh=True,\n        raise_on_error=False\n    )\n    count_news = client.count(index=INDEX_NEWS)[\"count\"]\n    print(\"Indexadas:\", ok_news)\n    print(\"Errores:\", len(errores_news))\n    print(\"Conteo Elasticsearch:\", count_news)\n    assert count_news == len(noticias) == 987\nelse:\n    count_news = None\n    print(\"Respaldo local. Noticias disponibles:\", len(noticias))"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## H4. Recibe tu brief editorial\n\nEl alias de tu equipo asigna una necesidad distinta, pero todas tienen suficientes antecedentes en el corpus.\n\nLa regla de negocio es común:\n\n> **solo puedes recomendar contenido de acceso abierto**.\n\nEso no es relevancia; es un filtro."
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "metadata": {},
   "outputs": [],
   "source": "BRIEFS = [\n    {\n        \"nombre\": \"Radar IA\",\n        \"consulta\": \"inteligencia artificial\",\n        \"criterio\": \"La noticia trata de forma sustantiva usos, impacto, regulación o riesgos de inteligencia artificial.\"\n    },\n    {\n        \"nombre\": \"Bienestar\",\n        \"consulta\": \"salud mental\",\n        \"criterio\": \"La noticia aborda directamente salud mental, bienestar psicológico, prevención o atención relacionada.\"\n    },\n    {\n        \"nombre\": \"Bogotá segura\",\n        \"consulta\": \"seguridad bogota\",\n        \"criterio\": \"La noticia aporta antecedentes directos sobre seguridad, delito, convivencia o respuesta institucional en Bogotá.\"\n    },\n    {\n        \"nombre\": \"Energía\",\n        \"consulta\": \"energia colombia\",\n        \"criterio\": \"La noticia trata directamente energía, abastecimiento, tarifas, generación o política energética en Colombia.\"\n    },\n    {\n        \"nombre\": \"Trabajo\",\n        \"consulta\": \"empleo colombia\",\n        \"criterio\": \"La noticia aporta información directa sobre empleo, mercado laboral, contratación o trabajo en Colombia.\"\n    },\n    {\n        \"nombre\": \"Universidad\",\n        \"consulta\": \"educacion superior\",\n        \"criterio\": \"La noticia trata directamente universidades, educación superior, acceso, financiación o calidad.\"\n    }\n]\n\nbrief_news = BRIEFS[sum(ord(c) for c in alias_seguro) % len(BRIEFS)]\n\nprint(\"BRIEF:\", brief_news[\"nombre\"])\nprint(\"Consulta:\", brief_news[\"consulta\"])\nprint(\"Criterio de relevancia:\", brief_news[\"criterio\"])\nprint(\"Regla de negocio: premium = False\")"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## H5. Ranking A — baseline editorial\n\nPrimero busca en título, subtítulo y cuerpo **sin boosts**.\n\nEl filtro <code>premium = false</code> se aplica en ambas configuraciones, porque es una condición del producto editorial, no una hipótesis de relevancia."
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "metadata": {},
   "outputs": [],
   "source": "def mostrar_hits_news(resp):\n    filas = []\n    for pos,h in enumerate(resp.get(\"hits\",{}).get(\"hits\",[]), start=1):\n        s = h.get(\"_source\",{})\n        hi = h.get(\"highlight\",{})\n        fragmentos = []\n        for piezas in hi.values():\n            fragmentos.extend(piezas)\n        filas.append({\n            \"rank\": pos,\n            \"id_noticia\": s.get(\"id_noticia\",\"\"),\n            \"score\": h.get(\"_score\"),\n            \"categoria\": s.get(\"categoria\",\"\"),\n            \"titulo\": s.get(\"titulo\",\"\"),\n            \"premium\": s.get(\"premium\",False),\n            \"highlight\": \" ... \".join(fragmentos),\n            \"url\": s.get(\"url\",\"\")\n        })\n    return pd.DataFrame(filas)\n\ndef buscar_news_elastic(campos):\n    return client.search(\n        index=INDEX_NEWS,\n        query={\n            \"bool\":{\n                \"must\":[{\n                    \"multi_match\":{\n                        \"query\":brief_news[\"consulta\"],\n                        \"fields\":campos\n                    }\n                }],\n                \"filter\":[{\"term\":{\"premium\":False}}]\n            }\n        },\n        highlight={\"fields\":{\"titulo\":{},\"subtitulo\":{},\"cuerpo\":{}}},\n        size=5\n    )\n\nif client is not None:\n    news_A = mostrar_hits_news(\n        buscar_news_elastic([\"titulo\",\"subtitulo\",\"cuerpo\"])\n    )\nelse:\n    news_base = noticias.loc[~noticias[\"premium\"]].copy()\n    news_local = buscar_bm25_local(\n        news_base.rename(columns={\"id_noticia\":\"id_proceso\"}),\n        brief_news[\"consulta\"]\n    ).head(5)\n    news_A = pd.DataFrame({\n        \"rank\": news_local[\"rank\"],\n        \"id_noticia\": news_local[\"id_proceso\"],\n        \"score\": news_local[\"score\"],\n        \"categoria\": news_local[\"categoria\"],\n        \"titulo\": news_local[\"titulo\"],\n        \"premium\": news_local[\"premium\"],\n        \"highlight\": \"\",\n        \"url\": news_local[\"url\"]\n    })\n\ndisplay(news_A)"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## H6. Ranking B — tuning editorial\n\nAhora formula una hipótesis de producto:\n\n> Si el tema aparece en el **título** o en las **etiquetas editoriales**, debería pesar más que una aparición perdida en el cuerpo.\n\nLa configuración B aplica esa decisión:\n\n- título × 4;\n- etiquetas × 3;\n- subtítulo × 2;\n- cuerpo × 1.\n\nTodo lo demás permanece constante."
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "metadata": {},
   "outputs": [],
   "source": "if client is not None:\n    news_B = mostrar_hits_news(\n        buscar_news_elastic([\n            \"titulo^4\",\n            \"etiquetas_texto^3\",\n            \"subtitulo^2\",\n            \"cuerpo\"\n        ])\n    )\nelse:\n    # Respaldo pedagógico aproximado: título y metadatos reciben más peso.\n    tmp_news = noticias.rename(columns={\"id_noticia\":\"id_proceso\"}).copy()\n    score_titulo = _bm25_scores(tmp_news[\"titulo\"], brief_news[\"consulta\"])\n    score_etiquetas = _bm25_scores(tmp_news[\"etiquetas_texto\"], brief_news[\"consulta\"])\n    score_subtitulo = _bm25_scores(tmp_news[\"subtitulo\"], brief_news[\"consulta\"])\n    score_cuerpo = _bm25_scores(tmp_news[\"cuerpo\"], brief_news[\"consulta\"])\n    local_B = _ordenar_scores(\n        tmp_news,\n        4*score_titulo + 3*score_etiquetas + 2*score_subtitulo + score_cuerpo\n    )\n    local_B = local_B[~local_B[\"premium\"]].head(5)\n    news_B = pd.DataFrame({\n        \"rank\": local_B[\"rank\"],\n        \"id_noticia\": local_B[\"id_proceso\"],\n        \"score\": local_B[\"score\"],\n        \"categoria\": local_B[\"categoria\"],\n        \"titulo\": local_B[\"titulo\"],\n        \"premium\": local_B[\"premium\"],\n        \"highlight\": \"\",\n        \"url\": local_B[\"url\"]\n    })\n\ndisplay(news_B)"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## H7. Evalúa como editor, no como programador\n\nUsa el criterio asignado por el brief y etiqueta los top 5 de A y B.\n\nNo cambies el criterio después de ver qué configuración “ganó”."
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "metadata": {},
   "outputs": [],
   "source": "print(\"Consulta:\", brief_news[\"consulta\"])\nprint(\"Criterio:\", brief_news[\"criterio\"])\n\netiquetas_news_A = etiquetar_top5(\n    news_A.rename(columns={\"id_noticia\":\"id_proceso\",\"titulo\":\"nombre_proceso\"}),\n    \"NOTICIAS A\"\n)\netiquetas_news_B = etiquetar_top5(\n    news_B.rename(columns={\"id_noticia\":\"id_proceso\",\"titulo\":\"nombre_proceso\"}),\n    \"NOTICIAS B\"\n)\n\np5_news_A = sum(etiquetas_news_A)/5\np5_news_B = sum(etiquetas_news_B)/5\n\nprint(\"Precision@5 noticias A:\", p5_news_A)\nprint(\"Precision@5 noticias B:\", p5_news_B)\nprint(\"Cambio:\", round(p5_news_B-p5_news_A,3))"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "## H8. Decisión editorial\n\nTu conclusión no puede ser solo “B tiene más score”.\n\nDebe responder:\n\n- ¿cuál ranking recomendarías a la mesa digital?;\n- ¿qué artículo de los cinco abrirías primero y por qué?;\n- ¿qué falso positivo encontraste?;\n- ¿el boost mejoró P@5 o solo cambió el orden?;\n- ¿qué consulta o filtro probarías después?\n\nEste reto demuestra transferencia: **mismo motor, documentos distintos, pregunta distinta, mismas ideas de diseño y evaluación**."
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "metadata": {},
   "outputs": [],
   "source": "RECOMENDACION_NEWS = \"¿A o B? Justifica con P@5 y al menos un resultado concreto.\" #@param {type:\"string\"}\nPRIMER_ARTICULO = \"ID/título del artículo que abrirías primero y por qué.\" #@param {type:\"string\"}\nFALSO_POSITIVO_NEWS = \"Identifica un resultado dudoso y explica qué coincidencia pudo traerlo.\" #@param {type:\"string\"}\nSIGUIENTE_PRUEBA_NEWS = \"Propón una sola mejora siguiente: boost, campo, filtro o consulta.\" #@param {type:\"string\"}\n\nreto_news = {\n    \"alias\": alias_seguro,\n    \"brief\": brief_news,\n    \"motor\": \"Elasticsearch real\" if client is not None else \"BM25 local de respaldo\",\n    \"index_name\": INDEX_NEWS if client is not None else None,\n    \"noticias\": int(len(noticias)),\n    \"precision_at_5_A\": p5_news_A,\n    \"precision_at_5_B\": p5_news_B,\n    \"recomendacion\": RECOMENDACION_NEWS,\n    \"primer_articulo\": PRIMER_ARTICULO,\n    \"falso_positivo\": FALSO_POSITIVO_NEWS,\n    \"siguiente_prueba\": SIGUIENTE_PRUEBA_NEWS\n}\n\nPath(\"s07_reto_sala_redaccion.json\").write_text(\n    json.dumps(reto_news, ensure_ascii=False, indent=2),\n    encoding=\"utf-8\"\n)\n\nprint(\"Reto exportado: s07_reto_sala_redaccion.json\")"
  },
  {
   "cell_type": "code",
   "execution_count": null,
   "metadata": {},
   "outputs": [],
   "source": "try:\n    from google.colab import files\n    files.download(\"s07_reto_sala_redaccion.json\")\nexcept ImportError:\n    print(\"Fuera de Colab: el archivo quedó en el directorio de trabajo.\")"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
   "source": "### Cierre del reto\n\nEl mismo conjunto de noticias ya había servido para aprender **modelo documental en MongoDB**. Ahora sirve para aprender **recuperación y ranking en Elasticsearch**.\n\nEse contraste es deliberado:\n\n- MongoDB responde bien cuando tu pregunta principal es guardar/consultar la estructura documental;\n- Elasticsearch aparece cuando necesitas analizar texto y ordenar documentos por relevancia;\n- la herramienta cambia porque **la pregunta cambia**.\n\nEl archivo <code>s07_reto_sala_redaccion.json</code> se suma a la evidencia de la sesión, pero este reto es de transferencia; no reemplaza el primer taller evaluativo de S8."
  },
  {
   "cell_type": "markdown",
   "metadata": {},
  ...
  },
  {
   ...
   "metadata": {},
   "outputs": [],
   "source": "BORRAR_MIS_INDICES = False #@param {type:\"boolean\"}\n\nif BORRAR_MIS_INDICES and client is not None:\n    for indice in [INDEX_NAME, INDEX_NEWS]:\n        if client.indices.exists(index=indice):\n            client.indices.delete(index=indice)\n            print(\"Índice eliminado:\", indice)\n        else:\n            print(\"Ya no existe:\", indice)\nelse:\n    print(\"Sin limpieza automática.\")"
  },
  {
   "cell_type": "markdown",
   "metadata": {},
