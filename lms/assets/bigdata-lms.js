...
async function s09(action='me',payload={}){if(action==='me'||action==='teacher_wall')return request('bigdata-session9?action='+encodeURIComponent(action),{method:'GET'});return request('bigdata-session9',{method:'POST',body:JSON.stringify({action,...payload})})}
async function core(action='home',payload={}){if(action==='home'||action==='teacher_overview')return request('bigdata-lms-core?action='+encodeURIComponent(action),{method:'GET'});return request('bigdata-lms-core',{method:'POST',body:JSON.stringify({action,...payload})})}
...
window.BIGDATA_LMS={API,STORE,ROOT,$,esc,auth,save,login,authAction,bigdata,s09,core,assess,interop,ops,requestAccess,reviewAccess,logout,fmtTime,fmtWhen,fmtDelay,safeNext,msg,requireBigData,wireLogout,startHeartbeat};
})();