...
function safeNext(){const p=new URLSearchParams(location.search),n=p.get('next');if(!n)return ROOT+'session-08.html';try{const u=new URL(n,location.origin);return u.origin===location.origin&&u.pathname.startsWith(ROOT)?u.pathname+u.search+u.hash:ROOT+'session-08.html'}catch{return ROOT+'session-08.html'}}
...
window.BIGDATA_LMS={API,STORE,ROOT,$,esc,auth,save,login,authAction,bigdata,requestAccess,reviewAccess,logout,fmtTime,fmtWhen,safeNext,msg,requireBigData,wireLogout,startHeartbeat};
})();