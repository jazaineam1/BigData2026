...
set title='SECOP Data Pipeline · API, concurrencia y NoSQL',
    metadata=jsonb_build_object(
      ...
      'max_score',100,
      'validator_version','2026-09-26-v4-secoppipeline'
    )
...
update public.bd_lms_activities
set metadata='{"validator_versions":["2026-09-17-v3-historico-atlas","2026-09-26-v4-secoppipeline"],"current":"2026-09-26-v4-secoppipeline"}'::jsonb
where code='bd-s08-final';

update public.lms_assignments_v2
set title='TC1 V4 · SECOP Data Pipeline',
    ...
    rubric='[
      {"code":"E1","title":"API SECOP + concurrencia + trazabilidad","max":20},
      {"code":"E2","title":"Modelo documental + Atlas idempotente","max":30},
      {"code":"E3","title":"Producto analítico","max":10},
      {"code":"E4","title":"Cassandra query-first","max":15},
      {"code":"E5","title":"Neo4j y contexto relacional","max":20},
      {"code":"E6","title":"Decisiones + informe reproducible","max":5}
    ]'::jsonb,
    updated_at=now()
